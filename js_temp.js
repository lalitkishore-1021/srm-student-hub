        ...
        window.addEventListener('load', () => {
            // Fix: Wait for DOM, but don't force a hash reload that ruins click events.
            const hash = window.location.hash.replace('#', '');
            if (hash && document.getElementById(hash)) {
                if (hash === 'music-view') loadMusicList();
        ...
        function renderProfile(profile) {
            ...
            if (avatarImg && profile.regNo) {
                // Real Indian Hero / Heroine avatars that change every day!
                const nameStr = (profile.name || profile.regNo).toLowerCase();
                
                // Gender detection from Indian names
                const girlNames = ["priya","shreya","neha","anjali","swati","pooja","shruti","aarti","divya","kavya","nandini","riya","isha","sneha","megha","lakshmi","sita","sri","ananya","tanya","devi","jyoti","meena","radha","geetha","bhavya","harini","sahana","varsha","ammu","keerthi","akshara","shalini","trisha","nithya","oviya","nazriya","rashmika","samantha","tamanna"];
                const isGirl = girlNames.some(n => nameStr.includes(n));
                const boyNames = ["lalit","kishore","kumar","raju","babu","sai","ram","krishna","rahul","amit","rohit","vikas","manish","balaga","arjun","vijay","surya","karthi","arun","karthik","pranav","harsha","nikhil","varun","deepak","suresh","ganesh","mahesh","raj","ajay","naveen","pavan","charan","dhoni","kohli","sachin"];
                const isBoy = boyNames.some(n => nameStr.includes(n)) || !isGirl;

                // Indian Heroes (Actors) - proxied for reliability
                const heroImages = [
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/6/6e/Shah_Rukh_Khan_graces_the_launch_of_the_new_Santro.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/0/09/Allu_Arjun_at_Pushpa_2_The_Rule_meet.jpg/500px-Allu_Arjun_at_Pushpa_2_The_Rule_meet.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/2/22/Prabhas_by_Gage_Skidmore.jpg/500px-Prabhas_by_Gage_Skidmore.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/9/9c/Hrithik_at_Rado_launch.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/3/32/Ranveer_Singh_in_2023_%281%29_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/8/82/Dulquer_Salmaan_at_Karwaan_promotions_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/a/a0/Ranbir_Kapoor_snapped_at_Kalina_airport.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Mahesh_Babu_in_Spyder_%28cropped%29.jpg/500px-Mahesh_Babu_in_Spyder_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/d/d3/Ram_Charan_at_Game_Changer_trailer_launch.jpg/500px-Ram_Charan_at_Game_Changer_trailer_launch.jpg"
                ];

                // Indian Heroines (Actresses) - proxied for reliability
                const heroineImages = [
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Alia_Bhatt_attends_at_the_2026_Cannes_Film_Festival_%28cropped%29_%28cropped%29.jpg/500px-Alia_Bhatt_attends_at_the_2026_Cannes_Film_Festival_%28cropped%29_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/4/4b/Rashmika-Mandanna_at_the_music_launch_of_Chhaava_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Samantha_at_Kushi_Musical_concert_%282%29_%28cropped%29.jpg/500px-Samantha_at_Kushi_Musical_concert_%282%29_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/e/e6/Anushka_Sharma_promoting_Zero.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/8/80/Nayanthara_at_Filmfare_Awards.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/7/7b/Shraddha_Kapoor_promoting_Street_Dancer_3D.jpg/500px-Shraddha_Kapoor_promoting_Street_Dancer_3D.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/d/d4/Kiara_Advani_snapped_at_the_screening_of_Shershaah_%28cropped%29.jpg/500px-Kiara_Advani_snapped_at_the_screening_of_Shershaah_%28cropped%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/thumb/5/59/Pooja_Hegde_at_Beast_Telugu_press_meet_%281%29.jpg/500px-Pooja_Hegde_at_Beast_Telugu_press_meet_%281%29.jpg",
                    "https://wsrv.nl/?url=upload.wikimedia.org/wikipedia/commons/6/60/Tamannaah_Bhatia_at_a_song_launch_from_the_film_Vedaa_%28cropped%29.jpg"
                ];


                // Pick image based on day of year (changes daily!)
                const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(), 0, 0)) / 86400000);
                const imageList = (isBoy && !isGirl) ? heroImages : heroineImages;
                const idx = dayOfYear % imageList.length;
                
                avatarImg.src = imageList[idx];
                ...
                avatarImg.onerror = function() {
                    const nameParam = encodeURIComponent(profile.name || profile.regNo || 'SRM');
                    this.src = `https://ui-avatars.com/api/?name=${nameParam}&background=random&color=fff&size=200`;
                };
            }
        ...
        function toggleBatch(element, batchNum) {
            // Function preserved to avoid JS errors if clicked, but UI is removed
        ...
        function renderAttendance(attData) {
            ...
            attendanceData.forEach(sub => {
                ...
                let pct = parseFloat(sub.attended) || 0;
                let C   = parseInt(sub.classes_per_cycle) || 1;
                ...
                for (let N = 1; N <= 30; N++) {
                    let t_held = N * C;
                    let c_att = Math.round((pct / 100) * t_held);
                    let c_pct = (c_att / t_held) * 100;
                    let diff = Math.abs(c_pct - pct);
                    if (diff < best_diff) {
                        best_diff = diff;
                        best_N = N;
                    }
                ...
                let baseTotal = best_N * C;
                let baseAttended = Math.round((pct / 100) * baseTotal);
                ...
                let simulated_pct_val = total === 0 ? 0 : (attended / total) * 100;
                const percentage = (sim.attended === 0 && sim.bunked === 0) 
                                 ? pct.toFixed(1) 
                                 : simulated_pct_val.toFixed(1);

                const isGood = simulated_pct_val >= TARGET * 100;
                ...
                let marginLabel = 'Safe Bunks';
                let marginValue = 0;
                
                if (isGood) {
                    marginValue = Math.floor((attended - TARGET * total) / TARGET);
                    marginValue = Math.max(0, marginValue);
                    marginLabel = 'Safe Bunks';
                } else {
                    marginValue = Math.ceil((TARGET * total - attended) / (1 - TARGET));
                    marginValue = Math.max(0, marginValue);
                ...
                html += `
                    <div class="image-card morph-card" oncontextmenu="handleLongPress(event, '${name}', 'attendance')" onmousedown="startLongPress(event, '${name}', 'attendance')" onmouseup="cancelLongPress()" onmouseleave="cancelLongPress()" ontouchstart="startLongPress(event, '${name}', 'attendance')" ontouchend="cancelLongPress()" ontouchcancel="cancelLongPress()" style="display: flex; align-items: center; justify-content: space-between; padding: 20px; margin-bottom: 15px; transition: all 0.3s; ${cardStyleModifier}">
                        ...
                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; background: rgba(0,0,0,0.3); border-radius: 12px; padding: 15px; border: 1px solid ${isGood ? 'rgba(0,204,102,0.3)' : 'rgba(255,68,68,0.3)'}; min-width: 80px; text-align: center;">
                            ...
                            <div style="font-size: 0.65rem; color: ${barColor}; font-weight: 800; text-transform: uppercase; margin-top: 4px; margin-bottom: 10px; letter-spacing: 0.5px; white-space: nowrap;">${marginLabel}</div>
        ...
        function renderTimetable(ttData) {
            ...
            if (!hasClasses) {
                ...
                noData.querySelector('p').innerHTML = "The scraper found zero classes. Your batch is automatically detected from your profile.";
                return;
            }
        ...
        async function loadLeaderboard(type) {
            ...
            try {
                ...
                list.innerHTML = data.map((s, i) => {
                    const val = type === 'attendance' ? `${s.overall_attendance}%` : `Score ${s.est_cgpa}`;
        ...
        async function loadDownloadsList() {
            ...
            let html = '';
            let savedOrder = JSON.parse(localStorage.getItem('downloadsOrder') || '[]');
            tracks.sort((a, b) => {
                ...
                if (idxA !== -1) return -1;
                if (idxB !== -1) return 1;
                return (b.downloaded_at || 0) - (a.downloaded_at || 0);
            });
            tracks.forEach((t, i) => {
                html += `
            ...
            window.dlSortable = Sortable.create(container, {
                ...
                onEnd: function(evt) {
                    ...
                    localStorage.setItem('downloadsOrder', JSON.stringify(newOrder));
        ...
        async function playOfflineTrack(trackId) {
            ...
            let savedOrder = JSON.parse(localStorage.getItem('downloadsOrder') || '[]');
            currentPlaylist = tracks.sort((a, b) => {
                let idxA = savedOrder.indexOf(a.id);
                let idxB = savedOrder.indexOf(b.id);
                if (idxA !== -1 && idxB !== -1) return idxA - idxB;
                if (idxA !== -1) return -1;
        ...
        audioPlayer.addEventListener('ended', () => {
            if (typeof handleMusicEnded === 'function') handleMusicEnded();
        });
        audioPlayer.addEventListener('error', () => {
            if (typeof handleMusicError === 'function') handleMusicError();
        });
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                if (currentTrackIndex !== -1 && audioPlayer.paused && !userManuallyStopped) {
                    audioPlayer.play().catch(() => {});
                }
            }
        });

        function renderTracks(playlistData, listContainer) {
            if (playlistData.length === 0) {
                listContainer.innerHTML = `<div style="text-align: center; color: var(--text-sub); padding: 40px;">No tracks found. Upload the first one!</div>`;
                return;
            }
            
            const myNetId = getCurrentNetId();
            listContainer.innerHTML = playlistData.map((p, index) => {
                const isOwner = myNetId && p.net_id && myNetId === (p.net_id || '').toLowerCase();
                ...
                <div class="music-card fade-in-up" data-id="${p.id}" onclick="playMusicTrack(${index})" style="transform: none; cursor: grab;">
                    ...
                    <div style="position: relative; width: 60px; height: 60px; flex-shrink: 0; margin-right: 15px; cursor: pointer; -webkit-touch-callout: none; -webkit-user-select: none; user-select: none;"
                         oncontextmenu="return false;">
                        <img src="${cover}" class="music-cover" alt="${p.title}" loading="lazy" onerror="this.src='data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMDAiIGhlaWdodD0iMTAwIj48cmVjdCB3aWR0aD0iMTAwJSIgaGVpZ2h0PSIxMDAlIiBmaWxsPSIjMzMzIi8+PC9zdmc+'" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; margin: 0; pointer-events: none;">
                        ${p.has_video ? `<video loop muted playsinline onclick="event.stopPropagation(); this.pause(); this.style.display='none'; this.parentElement.querySelector('.vid-toggle').style.display='flex';" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; display: none; border-radius: 12px; z-index: 2; cursor: pointer; pointer-events: auto;"></video>` : ''}
                        ${p.has_video ? `<div class="vid-toggle" onclick="event.stopPropagation(); const btn = this; const v = btn.parentElement.querySelector('video'); if(!v.src || v.src === window.location.href) { btn.innerHTML='...'; fetch('${BACKEND_URL}/api/music/video/${p.id}').then(r=>r.json()).then(d=>{ if(d.video_data){v.src=d.video_data; v.style.display='block'; v.play().catch(e=>{}); btn.style.display='none'; btn.innerHTML='▶';} }); } else { v.style.display='block'; v.play().catch(e=>{}); btn.style.display='none'; }" style="position: absolute; bottom: 4px; left: 4px; background: rgba(29, 185, 84, 0.8); border-radius: 50%; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; z-index: 3; color: white; font-size: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.5);">▶</div>` : ''}
                    </div>
        ...
        async function loadMusicList(forceRefresh = false) {
            const list = document.getElementById('music-list');
            if(!list) return;

            if (musicCache && !forceRefresh) {
                currentPlaylist = musicCache;
                renderTracks(musicCache, list);
                initMusicSortable(list);
                return;
            }

            list.innerHTML = `
                ${Array(6).fill(`
                <div class="music-skeleton-card" style="display:flex; align-items:center; padding:15px; background:rgba(255,255,255,0.03); border-radius:12px; margin-bottom:10px;">
                    <div class="skeleton-thumb" style="width:60px; height:60px; margin-right:15px;"></div>
                    <div style="flex:1;">
                        <div class="skeleton-line long" style="height:15px; width:70%; margin-bottom:10px;"></div>
                ...
                if (lastId && !isPlaying && currentTrackIndex === -1) {
                    ...
                    if (idx !== -1) {
                        ...
                        fetch(`${BACKEND_URL}/api/music/audio/${track.id}`).then(r=>r.json()).then(d=>{
                            if(d.audio_data) audioPlayer.src = d.audio_data;
                        });
                    }
                }
            } catch (e) {
        ...
        function initMusicSortable(list) {
            ...
            window.musicSortable = Sortable.create(list, {
                ...
                onEnd: async function (evt) {
                    ...
                    try {
                        ...
                        musicCache = currentPlaylist; // Sync cache
        ...
        async function playMusicTrack(index) {
            ...
            const playerImg = document.getElementById('player-cover');
            const playerVid = document.getElementById('player-video');
            const fsImg = document.getElementById('fs-cover');
            const fsVid = document.getElementById('fs-video');
            
            playerVid.style.display = 'none';
            playerVid.src = '';
            playerImg.style.display = 'block';
            playerImg.src = coverSrc;
            
            if (fsVid) { fsVid.style.display = 'none'; fsVid.src = ''; }
            if (fsImg) { fsImg.style.display = 'block'; fsImg.src = coverSrc; }
            ...
            if (track.has_video) {
                fetch(`${BACKEND_URL}/api/music/video/${track.id}`).then(r => r.json()).then(data => {
                    if (data.video_data && track.id === currentPlaylist[currentTrackIndex].id) {
                        if (playerVid) {
                            playerVid.src = data.video_data;
                            playerVid.style.display = 'block';
                            playerImg.style.display = 'none';
                            playerVid.play().catch(e=>{});
                        }
                        if (fsVid) {
                            fsVid.src = data.video_data;
            ...
            const playAudioData = async (audioData) => {
                ...
                const isStreamUrl = !audioData.startsWith('data:');
                ...
                const startPlayback = async () => {
                    try {
                        ...
                        isLoadingTrack = false;
                        // Do NOT auto-skip here; it causes jumps on mobile when play is blocked.
                        return;
                    }
                ...
                if (isStreamUrl) {
                    ...
                    await new Promise((resolve, reject) => {
                        ...
                        const onError = () => { cleanup(); reject(audioPlayer.error); };
                        const cleanup = () => {
                        ...
                        setTimeout(() => { cleanup(); reject(new Error('Audio load timeout')); }, 15000);
                    }).then(() => {
                        streamLoaded = true;
                    }).catch(e => {
                        console.error("Stream load failed:", e);
                    });
                    if (streamLoaded) await startPlayback();
                } else {
                    await startPlayback();
                }
            };
        ...
        function toggleMusicPlay() {
            ...
            const playerVid = document.querySelector('#music-list .music-card.playing .vid-toggle') ? document.querySelector('#music-list .music-card.playing video') : null;
            const fsVid = document.getElementById('fs-video-bg');
            
            if (isPlaying) {
                audioPlayer.pause();
                if(playerVid && playerVid.style.display !== 'none') playerVid.pause();
                if(fsVid && fsVid.style.display !== 'none') fsVid.pause();
            } else { 
                audioPlayer.play().catch(e => console.warn(e)); 
                if(playerVid && playerVid.style.display !== 'none') playerVid.play();
                if(fsVid && fsVid.style.display !== 'none') fsVid.play();
            }
            isPlaying = !isPlaying;
            updatePlayPauseBtn();
        }
        ...
        function handleMusicEnded() {
            ...
            if (isRepeat) {
                ...
                audioPlayer.play().catch(e => { console.error('Repeat play failed:', e); });
            } else {
                nextMusicTrack();
            }
        }
        ...
        function handleMusicError() {
            // Don't skip if we're in the middle of loading a new track
            if (isLoadingTrack) return;
            // Don't skip if src is empty (we clear it intentionally while loading new tracks)
            if (!audioPlayer.src || audioPlayer.src === '' || audioPlayer.src === window.location.href) return;
            console.warn('Audio element error, auto-skipping to next track...');
            isPlaying = false;
            updatePlayPauseBtn();
            setTimeout(() => { nextMusicTrack(); }, 1500);
        }
        ...
        function updateMusicProgress() {
            ...
            const favicon = document.getElementById('dynamic-favicon');
            if (favicon && !audioPlayer.paused) {
                updateFavicon(true);
            } else if (favicon && audioPlayer.paused) {
                updateFavicon(false);
            }
            if (audioPlayer.duration) {
                ...
                document.getElementById('player-time-current').innerText = formatTime(audioPlayer.currentTime);
                document.getElementById('player-time-total').innerText = formatTime(audioPlayer.duration);
        ...
        function seekMusic(e) {
            ...
            audioPlayer.currentTime = (e.offsetX / bar.clientWidth) * audioPlayer.duration;
        }
        ...
        async function submitMusic() {
            ...
            const videoFile = document.getElementById('music-video-file')?.files[0];
            ...
            const ctaBtn = document.getElementById('music-upload-cta');
            if (ctaBtn) { ctaBtn.innerHTML = '<span class="upload-spinner"></span> Uploading...'; ctaBtn.style.pointerEvents = 'none'; ctaBtn.style.opacity = '0.8'; }
            ...
            try {
                ...
                if (videoFile) {
                    if (videoFile.size > 20 * 1024 * 1024) {
                        status.innerText = "Video file too large (Max 20MB)";
                        status.style.color = 'var(--danger)';
                        if (ctaBtn) { ctaBtn.innerHTML = 'UPLOAD TRACK'; ctaBtn.style.pointerEvents = 'auto'; ctaBtn.style.opacity = '1'; }
                        return;
                    }
                ...
                const payload = {
                    title: title, artist: artist, audio_data: audioBase64, cover_data: coverData, video_data: videoData,
        ...
        async function submitChat() {
            ...
            try {
                ...
                if(data.success) {
                    ...
                    function typeWriter() {
                        if (i < formatted.length) {
                            if (formatted.charAt(i) === '<') isTag = true;
                            text += formatted.charAt(i);
                            if (formatted.charAt(i) === '>') isTag = false;
                            if (isTag) {
                                i++;
                                typeWriter();
                            } else {
                ...
                errorBubble.style.cssText = 'background: rgba(138,43,226,0.2); padding: 10px 15px; border-radius: 15px; border-bottom-left-radius: 0; align-self: flex-start; max-width: 80%; color: #ff4757;';
                errorBubble.innerText = "Error: Failed to connect to AI server. Please try again later.";
        ...
        // Dynamic atmospheres removed to fix laptop flickering
        ...
        // --- REAL APK DOWNLOAD LOGIC ---
        function downloadRealAPK() {
            // Pointing to the actual .apk file that will be hosted on the server
            const apkUrl = "/static/srm-hub.apk";
            
            // Create a temporary anchor element to trigger the download
            const a = document.createElement('a');
            a.href = apkUrl;
            a.download = "SRM-Student-Hub.apk";
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            
            // Also notify the user to allow unknown sources if needed
            setTimeout(() => {
                alert("Downloading the Real APK...\n\nNote: If the download fails, it means the 'srm-hub.apk' file has not been uploaded to the server yet. Once downloaded, you may need to allow 'Install from Unknown Sources' in your Android settings.");
            }, 1000);
        }
